...
const DETECTION_RULES = [
  {
    ...
    query: {
      bool: {
        must: [
          { term: { "path.keyword": "/api/login" } },
          { term: { "method.keyword": "POST" } },
          {
            bool: {
              should: [
                { wildcard: { "details.keyword": "*' OR *" } },
                { wildcard: { "details.keyword": "*--*" } },
                { wildcard: { "details.keyword": "*1=1*" } },
                { wildcard: { "details.keyword": "*admin'*" } },
              ],
            },
          },
        ],
  ...
  },
  {
    ...
    severity: "critical",
    description: "SQLi pattern in products search query",
    icon: "💉",
    query: {
      bool: {
        must: [
          { term: { "path.keyword": "/api/products" } },
          {
            bool: {
              should: [
                { wildcard: { "path.keyword": "*UNION*" } },
                { wildcard: { "path.keyword": "*SELECT*" } },
                { wildcard: { "path.keyword": "*DROP*" } },
                { wildcard: { "path.keyword": "*' OR *" } },
                { wildcard: { "path.keyword": "*1=1*" } },
  ...
  },
  {
    ...
    severity: "critical",
    description: "Unauthenticated access to /api/admin/users",
    icon: "🔓",
    query: {
      bool: {
        must: [
          {
            bool: {
              should: [
                ...
                { wildcard: { "path.keyword": "*/admin/users*" } },
              ],
            },
          },
  ...
  },
  {
    ...
    severity: "high",
    description: "Suspicious user_id manipulation in orders endpoint",
    icon: "🕵️",
    query: {
      bool: {
        must: [
          { wildcard: { "path.keyword": "*/orders*user_id=*" } },
        ],
        must_not: [
  ...
  },
  {
    ...
    severity: "high",
    description: "High rate of failed logins from same IP",
    icon: "🔨",
    agg_query: {
      size: 0,
      query: {
      ...
      aggs: {
        by_ip: {
          terms: { field: "ip.keyword", min_doc_count: 5 },
        },
      },
    },
  },
  {
    ...
    name: "Sensitive Data Exposure — Plain Password",
    severity: "critical",
    description: "Admin users endpoint returning password fields",
    icon: "🔑",
    query: {
      bool: {
        must: [
  ...
  },
  {
    ...
    query: {
      bool: {
        should: [
          { wildcard: { "path.keyword": "*../*" } },
          { wildcard: { "path.keyword": "*%2e%2e%2f*" } },
          { wildcard: { "path.keyword": "*etc/passwd*" } },
        ],
      },
    },
  },
  {
    ...
    severity: "medium",
    description: "Known scanner user-agent or path patterns",
    icon: "🤖",
    query: {
      bool: {
        should: [
          ...
          { wildcard: { "user_agent.keyword": "*burp*" } },
          { wildcard: { "path.keyword": "*/.git*" } },
          { wildcard: { "path.keyword": "*/.env*" } },
...
async function runDetections(since = "now-1h") {
  const results = [];
  for (const rule of DETECTION_RULES) {
    if (rule.agg_query) {
      const data = await esQuery(`${APP_INDEX},${NGINX_INDEX}`, rule.agg_query);
      ...
      const data = await esQuery(`${APP_INDEX},${NGINX_INDEX}`, body);
  ...
  const body = {
    ...
    aggs: {
      over_time: {
        ...
        aggs: {
          statuses: { terms: { field: "status.keyword" } },
        },
      },
    },
  };
  return esQuery(`${APP_INDEX},${NGINX_INDEX}`, body);
}
...
async function getTopIPs() {
  const body = {
    ...
    aggs: {
      top_ips: { terms: { field: "ip.keyword", size: 10 } },
    },
  };
  return esQuery(`${APP_INDEX},${NGINX_INDEX}`, body);
}
...
async function getRecentEvents() {
  return esQuery(`${APP_INDEX},${NGINX_INDEX}`, {
    size: 20,
    sort: [{ timestamp: { order: "desc" } }],
    query: { match_all: {} },
  });
...
function TimelineChart({ buckets }) {
  if (!buckets || buckets.length === 0) return <div style={{ color: "var(--dim)", fontSize: 12, textAlign: "center", padding: "20px 0" }}>No timeline data</div>;
  ...
  return (
    <div style={{ display: "flex", alignItems: "flex-end", gap: 3, height: 60, padding: "0 4px" }}>
      {buckets.slice(-24).map((b, i) => {
        ...
        return (
          <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }} title={`${time}: ${b.doc_count} events`}>
            <div style={{ width: "100%", height: h, background: b.doc_count > 10 ? "#ff3b3b" : "#4ec9b0", borderRadius: "2px 2px 0 0", opacity: 0.85 }} />
          </div>
        );
      })}
    </div>
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
        ...
        <span style={{ fontSize: 13, fontWeight: 600, color: rule.triggered ? SEV_COLOR[rule.severity] : "var(--text)", fontFamily: "var(--mono)" }}>
          {rule.name}
        </span>
      </div>
...
function EventRow({ event }) {
  const isAttack = event.status === "error" ||
    /union|select|drop|insert|1=1|'--|\.\.\//i.test(JSON.stringify(event)) ||
    /admin|sqlmap|nikto/i.test(JSON.stringify(event));
  return (
    <div style={{
      display: "grid",
      ...
      <span style={{
        color: event.method === "POST" ? "#f5c518" : event.method === "DELETE" ? "#ff3b3b" : "#4ec9b0"
      }}>{event.method || "—"}</span>
      ...
      <span style={{
        color: event.status >= 400 || event.status === "error" ? "#ff3b3b" :
          event.status >= 300 ? "#f5c518" : "#4ec9b0"
      }}>{event.status || event.http_status || "—"}</span>
    </div>
  );
}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontSize: 22, marginBottom: 6 }}>{rule.icon} {rule.name}</div>
          <span style={{
            fontSize: 11, fontWeight: 700, letterSpacing: 1,
            color: SEV_COLOR[rule.severity], background: SEV_BG[rule.severity],
            padding: "3px 8px", borderRadius: 4, textTransform: "uppercase",
          }}>{rule.severity}</span>
        </div>
        <button onClick={onClose} style={{
          background: "none", border: "none", color: "var(--dim)", fontSize: 20, cursor: "pointer"
        }}>×</button>
      </div>
      <div style={{ flex: 1, overflow: "auto", padding: 24 }}>
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 11, color: "var(--dim)", marginBottom: 6, textTransform: "uppercase", letterSpacing: 1 }}>Description</div>
          <div style={{ fontSize: 13, lineHeight: 1.6 }}>{rule.description}</div>
        </div>
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 11, color: "var(--dim)", marginBottom: 6, textTransform: "uppercase", letterSpacing: 1 }}>Stats</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {[
              ...
              <div key={k} style={{ background: "var(--card)", borderRadius: 8, padding: "12px 14px" }}>
                ...
                <div style={{ fontSize: 16, fontFamily: "var(--mono)", fontWeight: 700, color: k === "Status" && rule.triggered ? "#ff3b3b" : "var(--text)" }}>{v}</div>
              </div>
            ))}
          </div>
        </div>
        {rule.ips?.length > 0 && (
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {rule.ips.map((ip) => (
                <span key={ip} style={{ fontFamily: "var(--mono)", fontSize: 12, background: "rgba(255,59,59,0.15)", color: "#ff3b3b", padding: "3px 10px", borderRadius: 4 }}>{ip}</span>
              ))}
            </div>
          </div>
        )}
        {rule.sampleEvents?.length > 0 && (
          <div>
            <div style={{ fontSize: 11, color: "var(--dim)", marginBottom: 8, textTransform: "uppercase", letterSpacing: 1 }}>Sample Events</div>
            {rule.sampleEvents.map((e, i) => (
              <div key={i} style={{
                background: "var(--card)", borderRadius: 6, padding: 12, marginBottom: 8,
                ...
                <div><span style={{ color: "var(--dim)" }}>status: </span>{e.status || e.http_status || "—"}</div>
              </div>
            ))}
          </div>
        )}
        <div style={{ marginTop: 20 }}>
          <div style={{ fontSize: 11, color: "var(--dim)", marginBottom: 8, textTransform: "uppercase", letterSpacing: 1 }}>ES Query</div>
          <pre style={{
            background: "var(--card)", borderRadius: 6, padding: 12,
            fontFamily: "var(--mono)", fontSize: 10, overflow: "auto",
            border: "1px solid var(--border)", color: "var(--dim)", lineHeight: 1.5
          }}>
...
export default function SecurityDashboard() {
  ...
  const filtered = filter === "active" ? detections.filter((d) => d.triggered)
    : filter !== "all" ? detections.filter((d) => d.severity === filter)
      : detections;
  ...
  return (
    <>
      ...
      {/* Notifications */}
      <div style={{ position: "fixed", top: 16, right: selectedRule ? 424 : 16, zIndex: 200, display: "flex", flexDirection: "column", gap: 8 }}>
        {notifications.map((n) => (
          <div key={n.id} className="notif" style={{
            background: SEV_BG[n.sev], border: `1px solid ${SEV_COLOR[n.sev]}`,
            borderRadius: 8, padding: "10px 16px", fontSize: 13,
            color: SEV_COLOR[n.sev], fontFamily: "var(--mono)", fontWeight: 600,
      ...
      <div style={{ minHeight: "100vh", background: "var(--bg)", display: "flex", flexDirection: "column" }}>
        ...
        <header style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <div style={{ fontFamily: "var(--mono)", fontSize: 18, fontWeight: 700, letterSpacing: -0.5 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12 }}>
              <div style={{
                ...
                background: esStatus === "connected" ? "#4ec9b0" : esStatus === "disconnected" ? "#ff3b3b" : "#f5c518",
        ...
        <div style={{ flex: 1, padding: 24, maxWidth: selectedRule ? "calc(100% - 400px)" : "100%", transition: "max-width 0.3s" }}>
          {/* Status bar */}
          {allActive.length > 0 && (
            <div style={{
              ...
              <span style={{ fontFamily: "var(--mono)", fontSize: 13, fontWeight: 700, color: "#ff3b3b" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 24 }}>
            {[
              ...
              <div key={k.label} className="card-hover" style={{
                ...
                <div style={{ fontSize: 36, fontWeight: 800, color: k.color, fontFamily: "var(--mono)", lineHeight: 1 }}>
                  {loading ? "—" : k.value}
                </div>
              </div>
            ))}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 20, marginBottom: 20 }}>
            ...
            <div>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                <div style={{ fontFamily: "var(--mono)", fontSize: 13, fontWeight: 700, color: "var(--dim)", textTransform: "uppercase", letterSpacing: 1 }}>
                  Detection Rules
                </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                {loading ? (
                  Array(8).fill(0).map((_, i) => (
                    <div key={i} style={{ background: "var(--card)", borderRadius: 8, height: 100, border: "1px solid var(--border)", opacity: 0.5 }} />
                  ))
                ) : (
                  filtered.map((rule) => (
                    <AlertCard key={rule.id} rule={rule} onClick={setSelectedRule} isActive={selectedRule?.id === rule.id} />
                  ))
                )}
              </div>
            </div>
                  ...
                  topIPs.slice(0, 8).map((b) => (
                    <div key={b.key} style={{ marginBottom: 10 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                        <span style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--text)" }}>{b.key}</span>
          ...
          <div style={{ marginTop: 20, textAlign: "center", color: "var(--dim)", fontSize: 11, fontFamily: "var(--mono)" }}>
            sec/watch · polling every {POLL_MS / 1000}s · ES: {ES_BASE}
          </div>
        </div>
      </div>
